...
//  EmojiMachine
//
//  Created by Shuai Hao on 2019/3/5.
...
class ViewController: UIViewController {
    ...
    @IBAction func goButttonDidTouch(_ sender: UIButton) {
        ...
        let buttonWidth = self.goButton.bounds.width
        UIView.animate(withDuration: 0.5, delay: 0.0, usingSpringWithDamping: 0.1, initialSpringVelocity: 5, options: .curveLinear, animations: {
            self.goButton.bounds = CGRect(x: self.bounds.origin.x, y: self.bounds.origin.y, width: buttonWidth - 20, height: self.bounds.size.height)
        }) { (_) in
            UIView.animate(withDuration: 0.1, animations: {
                self.goButton.bounds = CGRect(x: self.bounds.origin.x, y: self.bounds.origin.y, width: buttonWidth, height: self.bounds.size.height)
            })
        }
    }
